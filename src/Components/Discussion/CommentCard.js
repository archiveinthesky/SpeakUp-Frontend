...
const CommentCard = forwardRef(({ boardId, onSide, cmtdata, motherComment = null, APIPostReply, delComment }, ref) => {
        ...
        return (
            <span className="absolute top-0 left-0 w-screen h-screen" onClick={() => { setShowExtendedMenu(false) }}>
                <div className="absolute flex flex-col"
                    style={{ top: cardmenu.current.getBoundingClientRect().y + 35, right: window.innerWidth - cardmenu.current.getBoundingClientRect().x - 15 }}
                >
                    <button
    ...
    return (
        <>
            <div className="lg:w-11/12 my-2 lg:mx-auto lg:border-2 lg:border-gray-200 rounded-3xl duration-300" ref={ref}>

                <div className="w-full px-4 2xl:px-8 mx-auto mt-2 flex justify-start">
                    <img className="p-2 rounded-full overflow-hidden w-10 h-10 lg:w-14 lg:h-14" src={ProfileImg} alt="Profile" />
                    <div className="my-auto pl-2"><h3 className=" text-black text-lg lg:text-xl">{cmtdata.accName}</h3></div>
                </div>

                <p className="w-full px-8 2xl:px-16 mt-2 mb-1 lg:mb-4 mx-auto text-lg lg:text-xl">{cmtdata.cmtContent}</p>

                <div className="w-full px-8 2xl:px-16 mx-auto flex justify-between mt-2 mb-2">
                    <div className="flex h-10">
                        <div className="min-w-max h-8 lg:h-10 lg:px-2 lg:border-2 lg:border-gray-200 rounded-3xl flex items-center gap-1">
                            <div className='flex items-center'>
                                <button onClick={() => { updateUserStatus("liked") }}>
                                    <svg className={`inline mx-2 w-5 lg:w-8 ${(liked && enableAnim) && "animate-jump-up"} ${liked ? "filter-green ease-in" : "filter-none"}`} xmlns="http://www.w3.org/2000/svg" width="25.149" height="25.501" viewBox="0 0 25.149 25.501">
                                ...
                                <p className="inline text-lg mr-2">{cmtdata.cmtLikes + (liked ? 1 : 0)}</p>
                            </div>

                            <div className="flex items-center">
                                <button className='mx-2 overflow-hidden' onClick={() => { updateUserStatus("supported") }}>
                                    <ArrowCircleUpIcon className={`inline w-7 lg:w-8 overflow-hidden ${supported ? "text-blue-600 " : "text-blue"} ${supported & enableAnim && "animate-fly-up "}`} />
                                </button>
                            ...
                            <div className="flex items-center">
                                <button onClick={() => { updateUserStatus("disliked") }}>
                                    ...
                                    <p className="inline text-lg mr-2">{cmtdata.cmtDislikes + (disliked ? 1 : 0)}</p>
                                </button>
                            </div>
                        </div>
